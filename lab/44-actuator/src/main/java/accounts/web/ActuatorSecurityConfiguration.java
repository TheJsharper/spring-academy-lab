...
public class ActuatorSecurityConfiguration {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		// @formatter:off
		EndpointRequestMatcher endpointHealth = EndpointRequest.to(InfoEndpoint.class,HealthEndpoint.class);
		EndpointRequestMatcher endpointReport = EndpointRequest.to(ConditionsReportEndpoint.class);
		EndpointRequestMatcher endpointAnyRquests = EndpointRequest.toAnyEndpoint();
        http.authorizeHttpRequests((authz) -> authz
                .requestMatchers(endpointHealth).permitAll()
                .requestMatchers(endpointReport).hasRole("ADMIN")
                .requestMatchers(endpointAnyRquests).hasRole("ACTUATOR")
