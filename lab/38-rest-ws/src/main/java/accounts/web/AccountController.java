...
public class AccountController {
	...
	private ResponseEntity<Void> entityWithLocation(Object resourceId) {
		...
		UriComponents uriComponents = UriComponentsBuilder.newInstance().scheme("http").host("localhost").port(8080)
				.path("/accounts/" + resourceId).build();
		return ResponseEntity.created(uriComponents.toUri()).build(); // Return something other than null
	}
	...
	public ResponseEntity<Void> addBeneficiary(@PathVariable("accountId") long accountId,
		...
		UriComponents uriComponents = UriComponentsBuilder.newInstance().scheme("http").host("localhost").port(8080)
				.path("/accounts/" + accountId + "/beneficiaries/" + beneficiaryName).build();
		return ResponseEntity.created(uriComponents.toUri()).build(); // Modify this to return something
	}
	...
	public void handleAlreadyExistData(Exception ex) {
